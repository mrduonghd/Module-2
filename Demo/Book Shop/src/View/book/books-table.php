...
<table class="table table-striped " style="border-collapse: unset">
    ...
    <?php foreach ($books as $key => $book): ?>
        <tr>
            <td><a style="text-decoration: none" href="index.php?page=book-details&id=<?php echo $book['book_id'] ?>"><?php echo $book['book_name'] ?></a></td>
            <td><?php echo $book['author_name'] ?></td>
            <td><?php echo $book['category_name'] ?></td>
            <td><?php echo $book['price_sale'] ?></td>
            <td><img src="images/<?php echo $book['image'] ?>" alt="" style="width: 50px; height: 50px"></td>
            <td style="text-align: center">
                <div>
                    <a  style="margin: 0 0.5rem" class="btn btn-warning btn-sm" href="index.php?page=update-book&id=<?php echo $book['book_id'] ?>" >update</a>
                    <a onclick="return confirm(' Xác nhận đầu sách <?php echo $book['book_name'] ?>')" style="margin: 0 0.5rem" class="btn btn-danger btn-sm" href="index.php?page=delete-book&id=<?php echo $book['book_id'] ?>" >delete</a>
                </div>
            </td>
        </tr>
    <?php endforeach; ?>
